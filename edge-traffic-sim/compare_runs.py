...
def load_agg(path: Path):
    ...
    # per-tick averages across all intersections
    g = df.groupby('t_s', as_index=True)
    out = pd.DataFrame({
        'avg_q': g[['q_ns','q_ew']].mean().mean(axis=1),   # mean of q_ns and q_ew then mean across ints
        ...
        'sla_miss_rate': g['sla_miss'].mean(),  # if sla_miss is 0/1 per row
    })
...
def main():
    ...
    common_idx = timer.index.intersection(act.index)
